...
class PRAnalyzer:
    def __init__(self):
        ...
        self._init_notion()     # Init the Notion client
        self._register_tools()  # Register the MCP tools
    
    def _init_notion(self):     #Init the Notion client with the API key and the UUID page ID
        try:
    ...
    def _register_tools(self):  #Register MCP tools for PR analysis
        @self.mcp.tool()
        async def fetch_pr(repo_owner: str, repo_name: str, pr_number: int) -> Dict[str, Any]: #Fetch changes from a GitHub pull request
        ...
        @self.mcp.tool()
        async def create_notion_page(title: str, content: str) -> str: #Create a Notion page with PR analysis
            print(f"Creating the Notion page: {title}", file=sys.stderr)
            try:
    ...
    def run(self):      #Start the MCP server
        try:
            print("Running the MCP Server for GitHub PR Analysis...", file=sys.stderr)
